...
def synthesize_one_sample(args, result_path, text, spk, configs, i=0):
    ...
    if args.speaker_id:
        write(
            f'{result_path}/sample_pt{str(args.restore_epoch)}_sp{args.speaker_id}_t{args.timesteps}_{i}.wav',
            sample_rate, audio
        )
    else:
        write(
            f'{result_path}/sample_pt{str(args.restore_epoch)}_t{args.timesteps}_{i}.wav',
            sample_rate, audio
        )
...
if __name__ == "__main__":
    ...
    # with open('./syn_NEB_args.txt', 'r') as f:
    ...
    if args.mode == "batch":
        ...
        # with open("../frg2p/2023-FH1_submission_directory/NEB_test_par/NEB_test_par.txt", 'r', encoding='utf-8') as f:
            ...
            for line in f.readlines():
                ...
                phone.append(line.strip().split('|')[2])
        with torch.no_grad():
            for i, text in tqdm(enumerate(phone)):
                synthesize_one_sample(args, result_path, text, spk, configs, name[i])
