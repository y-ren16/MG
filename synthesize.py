...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source",
        type=str,
        default="resources/filelists/synthesis_chinese.txt",
    ...
    generator.load_state_dict(
        torch.load(
            os.path.join(
                '../MG-Data/output/ckpt', preprocess_config["dataset"], "2023-03-11-01_51",
    ...
    if args.mode == "batch":
        ...
        with torch.no_grad():
            for i, text in enumerate(texts):
                ...
                text_lens = torch.LongTensor(len(x))
                speakers = spk
                phones = x
                ...
                batchs = to_device(batchs, device)

                y_enc, y_dec, attn = generator(*(batchs[0][2:]), n_timesteps=args.timesteps, temperature=1.5,
                                                    stoc=False, length_scale=0.91)
                t = (dt.datetime.now() - t).total_seconds()
                sample_rate = 22050
                print(f'Grad-TTS RTF: {t * sample_rate / (y_dec.shape[-1] * 256)}')
    ...
    if args.mode == "single":
        with torch.no_grad():
            ...
            if language == "ch":
                text = preprocess_mandarin(text)
                x = torch.LongTensor(
                    intersperse(text_to_sequence(language, True, text, cleaners), len(symbols_ch))).cuda()[None]
            elif language == "en":
                x = torch.LongTensor(
                    intersperse(text_to_sequence(language, False, text, cleaners), len(symbols_en))).cuda()[None]
            elif language == "fr":
                x = torch.LongTensor(
                    intersperse(text_to_sequence(language, True, text, cleaners), len(symbols_fr))).cuda()[None]
            ...
            t = dt.datetime.now()
            batchs = [(ids, raw_texts, speakers, phones, text_lens, text_lens)]
            y_enc, y_dec, attn = generator(*(batchs[2:]), out_size=None, n_timesteps=args.timesteps, temperature=1.5,
                                           stoc=False, length_scale=0.91)
            t = (dt.datetime.now() - t).total_seconds()
            sample_rate = 22050
            print(f'Grad-TTS RTF: {t * sample_rate / (y_dec.shape[-1] * 256)}')
